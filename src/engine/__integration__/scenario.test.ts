...
import { calculateFlowRate, calculateSegment, derivedQ, buildChildrenMap, findMainCircuit, recommendPump } from '../hydraulics'
...
describe('Phase 4 integration: canonical 3-room scenario', () => {
  ...
  it('hydraulics: 3 segments, finds main circuit, recommends pump', () => {
    ...
    const segments: Record<string, Segment> = {
      's1': {
        id: 's1', systemId: SYS, parentSegmentId: null, name: 'Магистраль', equipmentId: null, qOverride: null,
        lengthM: 10, pipeId: 'steel-vgp-dn20', dnMm: null, kmsCounts: { elbow: 2 }, velocityTargetMS: 0.6,
        kind: 'trunk', autoGenerated: false,
      } as unknown as Segment,
      's2': {
        id: 's2', systemId: SYS, parentSegmentId: 's1', name: 'К гостиной', equipmentId: 'eq1', qOverride: null,
        lengthM: 6, pipeId: 'steel-vgp-dn20', dnMm: null, kmsCounts: { tee: 1 }, velocityTargetMS: 0.3,
        kind: 'connection', autoGenerated: false,
      } as unknown as Segment,
      's3': {
        id: 's3', systemId: SYS, parentSegmentId: 's1', name: 'К кухне', equipmentId: 'eq2', qOverride: null,
    ...
    // Build children map once, then derive Q for each segment
    const childrenMap = buildChildrenMap(segments, SYS)
    const qS1 = derivedQ('s1', segments, equipmentQMap, childrenMap)
    const qS2 = derivedQ('s2', segments, equipmentQMap, childrenMap)
    const qS3 = derivedQ('s3', segments, equipmentQMap, childrenMap)
